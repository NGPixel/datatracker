...
$(document).ready(function () {
  $('#content a')
    .each(function () {
      ...
      const newtext = text.replace(/([@._])/g, '$1<wbr>')
      if (newtext === text) {
        return
      }
  ...
  $.ajax({
    ...
    success: function (data) {
      for (const parentId in data) {
        const attachTo = $('.group-menu.group-parent-' + parentId)
        ...
        attachTo.find('.dropdown-menu')
          .remove()
        const menu = ['<ul class="dropdown-menu ms-n1 mt-n1">']
        const groups = data[parentId]
        let gtype = ''
        for (var i = 0; i < groups.length; ++i) {
          const g = groups[i]
          if (g.type != gtype) {
            if (i > 0) { menu.push('<li class="dropdown-divider"></li>') }
